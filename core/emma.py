...
from core.operations_checkers import Checkers
from core.operations_input_extractor import OperationInputExtractor
import os
import webbrowser
...
class Emma():
    ...
    def listen(self):
        ...
        self.__mode = modes.Modes.listening
        r = sr.Recognizer()
        ...
        for checker in Checkers.all_public_checkers():
            ...
            if checker_result == True:
                ...
                if operation_parameter_count > 1:
                    ...
                    input_manipulation_func = OperationInputExtractor.get_input_manipulator_by_operation_name(
                        operation_name)
    ...
    def __ready_mode_processing(self):
        ...
        '''
        self.__mode = modes.Modes.processing
        ...
        self.__after_processing()
        if self.__mode != modes.Modes.command:
            self.__mode = modes.Modes.ready
    ...
    def stop(self):
        self.__mode = modes.Modes.stopped
    ...
    def command_mode(self):
        '''set mode to command'''
        if self.__os_name == "Linux":
            self.__mode = modes.Modes.command
    ...
    def processing(self):
        ...
        if self.__mode != modes.Modes.stopped:
            # Command mode
            if self.__mode == modes.Modes.command:
                self.__command_mode_processing()
            # Ready mode
            elif self.__mode == modes.Modes.ready:
                # Get text input
